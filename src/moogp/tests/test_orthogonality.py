...
def test_cstar_orthogonal_to_intercept():
    ...
    I0 = trapezoid(Cstar, x=X[:,0], axis=0)
...
def test_cstar_orthogonal_to_intercept_and_linear():
    ...
    # \int c(x,x')
    I0 = trapezoid(Cstar, x=xs, axis=0)
    
    # \int x * c(x,x')
    I1 = trapezoid(Cstar * xs[:, None], x=xs, axis=0)
...
def _numeric_h_and_H_full(X_row, ell, sigma2, terms, grid_n=401):
    ...
    for j in range(d):
        ...
        h0 = trapezoid(kernel_x, x=grid)
        h1 = trapezoid(grid * kernel_x, x=grid)
        h_moments.append((h0, h1))

        K = np.exp(-((grid[:, None] - grid[None, :]) / ell[j]) ** 2)
        H00 = trapezoid(trapezoid(K, x=grid, axis=0), x=grid)
        H10 = trapezoid(trapezoid(grid[:, None] * K, x=grid, axis=0), x=grid)
        H01 = trapezoid(trapezoid(K * grid[None, :], x=grid, axis=0), x=grid)
        H11 = trapezoid(
            trapezoid(grid[:, None] * K * grid[None, :], x=grid, axis=0),
            x=grid,
        )
